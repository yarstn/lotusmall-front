...
// ما نحتاج revalidate في صفحة كلاينت

type ListingEx = Listing & {
  sellerOrigin?: string;
  category?: string;
...
export default function HomePage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const data = (await getListings()) as ListingEx[];
        ...
        setError("تعذّر جلب المنتجات. تأكدي أن السيرفر شغّال.");
        setListings([]);
      } finally {
        setLoading(false);
      }
      ...
      <div className="w-full border-y border-black/5 bg-gradient-to-r from-[#f9d67a] via-[#f5cd82] to-[#f2b564] text-gray-900 py-2">
        <div className="marquee-wrap">
          <div className="marquee-track font-semibold">
            <span>
              🇻🇳 Đây là nền tảng thương mại thử nghiệm miễn phí. • Đây là nền tảng
              thương mại thử nghiệm miễn phí. • Đây là nền tảng thương mại thử
              nghiệm miễn phí.
            </span>
            <span aria-hidden>
              🇻🇳 Đây là nền tảng thương mại thử nghiệm miễn phí. • Đây là nền tảng
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[16rem_minmax(0,1fr)_16rem] gap-6">
            ...
            <aside className="hidden lg:block space-y-6">
              ...
              <div className="rounded-[28px] p-5 shadow-lg bg-white border">
                ...
                <p className="text-sm text-gray-600">
                  Catch the latest deals and drops.
                </p>
              </div>
            </aside>
            ...
            <section className="space-y-8">
              ...
              <div>
                ...
                {loading ? (
                  <div className="text-center text-gray-500">جارٍ تحميل المنتجات…</div>
                ) : error ? (
                  <div className="text-center text-red-600">{error}</div>
                ) : listings.length === 0 ? (
                  <div className="text-center text-gray-500">
                    ما فيه منتجات حاليًا.
                  </div>
                ) : (
                  <div className="grid gap-6 sm:gap-8 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    {listings.map((item) => {
                      const raw = item.imageUrls?.[0] ?? "/placeholder.png";
                      const imgSrc = raw.startsWith("http://localhost:8080")
                      ...
                      return (
                        <div key={item.id} className="group">
                          ...
                          <div className="mt-3 text-xs sm:text-sm text-gray-500">
                            {(item as any).category ?? "Smartwatches"}
                          </div>
